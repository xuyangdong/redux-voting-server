...
export default function reducer(state = INITIAL_STATE, action) {
  switch (action.type) {
    // Prepare stage.
    case 'JOIN_GAME':
      let result = genPlayer(state.get('nameDic'))
      return state.updateIn(['player', action.clientId], player => player || result[0]).updateIn(['nameDic','decorator'],list => list.delete(result[1]))
    case 'TOGGLE_READY':
